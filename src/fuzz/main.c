...
void test_document_scanf(struct flags_t *flags, const char *data, size_t size) {
  (void)flags;   /* scanf() takes its format from the input, not from the flags */
  ...
  int nclauses = size ? 1 + ((unsigned char)data[0] % TEST_SCANF_MAX_CLAUSES) : 1;
  ...
  for (int i = 0; i < nclauses; i++) {
    ...
    unsigned char sel = size ? (unsigned char)data[i % size] : (unsigned char)i;
...
void test_fy_parser_parse_fp_checkpoint_rollback(struct flags_t *flags, const char *data, size_t size) {
  struct fy_parser *fyp = NULL;
  ...
  if (rc == 0) {
    ...
    fy_parse_compose(fyp, compose_inspect_cb, NULL); //TODO: check if this is needed
