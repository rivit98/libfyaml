...
# reported. The logs in $LOG_DIR are the product: reading them is
...
#
# Log name: [<TC>__][<lane>__]<artifact>__<variant>.log - the TC prefix only
# when TC is set, the lane only for an artifact that sits in a campaign's
...
\$LOG_DIR/[<TC>__][<lane>__]<artifact>__<variant>.log - the <TC>__ prefix only
when TC is set, the <lane>__ one only for an artifact inside a campaign's
<lane>/{crashes,hangs,queue}/, whose name is unique only there. A variant whose
log already exists is skipped. With VERBOSE set the log also names each test
case as it runs, its timing and any injected allocation failure - the way to
tell which test case a hang is stuck in.
...
replay_one() { # replay_one <variant> <logfile>
  ...
  san_env="$(triage_env_for "$v")"
  {
    ...
    ( afl_run $(env_args "$san_env" \
        ...
        "timeout $REPLAY_TIMEOUT $(printf '%q %q' "$(cpath "$(bin_of "$v")")" "$(cpath "$artifact")")" ) 2>&1 || rc=$?
...
# The log names, lane and TC prefix included, come from common.sh: triage-all
...
for v in "${variants[@]}"; do
  if [ -s "${log_prefix}__${v}.log" ]; then
    log "$(basename "$artifact"): ${v} already logged"
  else
    log "replaying $(basename "$artifact") through: ${v}"
