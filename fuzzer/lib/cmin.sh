...
#
#   ./a cmin            # afl-cmin over every instance queue
#   ./a cmin --tmin     # also afl-tmin each survivor (slow, rarely worth it)
#
...
afl_run -n "$CONTAINER_PREFIX-cmin" $(env_args "${AFL_ENV[@]}") -- \
...
log "next campaign: SEED_DIR=$CMIN_DIR ./a run   (or ./a seeds --force)"
rm -rf "$ALL"
