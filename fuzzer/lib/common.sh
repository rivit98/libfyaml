...
TRIAGE_VARIANTS=(asan msan lsan fast)
...
#
#   $LOG_DIR/[<TC>__][<lane>__]<artifact>
#
# With TC set the name starts with it. A campaign artifact is
...
triage_log_prefix() {
  ...
  printf -v "$out" '%s' "$LOG_DIR/${TC:+${TC}__}${lane:+${lane}__}${name}"
}
