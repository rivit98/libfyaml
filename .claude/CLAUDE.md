...
- Reuse existing builds (`build-repro`, `build-repro-msan`, ...) whenever
  possible: rebuild incrementally, do not reconfigure or start from scratch.
...
incrementally. If `build-repro` / `build-repro-msan` already exists, skip the
`cmake -S . -B ...` configure step and run only `cmake --build`; configure
from scratch only when the directory is missing or its cache does not match
the settings below.
